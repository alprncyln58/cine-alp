...
export default function App() {
    ...
    try {
      ...
      if (activeCategory === 'trending' || !heroMovie) {
        ...
        if (validBackdrops.length > 0) {
          setHeroMovie(
            validBackdrops[
              Math.floor(Math.random() * Math.min(5, validBackdrops.length))
            ]
          );
        }
      }
  ...
  const searchMovies = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/search/multi?api_key=${DIRECT_API_KEY}&language=tr-TR&query=${query}`
      );
      const data = await res.json();
      setMovies(
        data.results.filter(
  ...
  const handleInputChange = (e) => {
    ...
    setAuthError(''); // Hata varsa temizle
  };

  // Veritabanı (LocalStorage) Simülasyonu
  const getUsers = () =>
    JSON.parse(localStorage.getItem('CineAlp_users') || '[]');
  ...
  const handleRegister = (e) => {
    ...
    setTimeout(() => {
      ...
      const newUser = {
        ...
        password: formData.password, // Gerçek dünyada şifreler hash'lenmelidir!
  ...
  // GOOGLE GİRİŞ SİMÜLASYONU
  const handleGoogleLogin = () => {
    setAuthLoading(true);
    // Google popup simülasyonu
    setTimeout(() => {
      const googleUser = {
        name: 'Alperen',
        surname: '(Google)',
  ...
  const getVideoSource = (movie) => {
    if (!movie) return '';
    const type =
      movie.media_type === 'tv' || (!movie.media_type && !movie.title)
        ? 'tv'
        : 'movie';
    const id = movie.id;
  ...
  const getImage = (path, size = 'original') =>
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white font-sans selection:bg-red-600 selection:text-white pb-10">
      {/* --- HEADER --- */}
      <header
        className={`fixed top-0 w-full z-[60] transition-all duration-300 flex flex-col ${
          scrolled
            ? 'bg-[#141414]/95 backdrop-blur-xl shadow-lg shadow-black/50'
      ...
      >
        <div className="px-4 md:px-10 h-16 flex items-center justify-between">
          ...
          <div className="flex gap-4 items-center">
            {/* Arama */}
            <div
              className={`hidden md:flex items-center bg-black/40 border ${
                query ? 'border-red-600' : 'border-white/20'
              } rounded-full px-3 py-1.5 transition-all duration-300 focus-within:border-red-600 w-64 backdrop-blur-sm`}
            >
              ...
              <input
                ...
                className="bg-transparent border-none outline-none text-sm text-white placeholder-gray-500 ml-2 w-full"
              />
              {query && (
            ...
            {user ? (
              <div className="relative">
                <div
                  ...
                  className="flex items-center gap-2 cursor-pointer hover:bg-white/10 p-1.5 rounded-full transition"
                >
                  <img
                    src={user.avatar}
                    alt="Profile"
                ...
                {showProfileMenu && (
                  <div className="absolute right-0 top-12 w-48 bg-[#181818] border border-gray-700 rounded-lg shadow-xl py-2 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    >
                      <Heart size={16} /> Listem ({myList.length})
                    </button>
                    <button
                      onClick={() => {
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2 hover:bg-red-900/30 text-red-500 flex items-center gap-2 text-sm"
                    >
          ...
          <div className="relative px-4 md:px-10 pb-3 group w-full border-b border-white/5">
            <div className="flex items-center">
              <button
                ...
                className="absolute left-0 z-20 bg-gradient-to-r from-[#141414] to-transparent px-4 h-full hidden md:flex items-center opacity-0 group-hover:opacity-100 transition"
              >
                <ChevronLeft size={24} />
              </button>
              <div
                ref={categoryScrollRef}
                className="flex gap-3 overflow-x-auto scrollbar-hide scroll-smooth w-full pr-4 md:pr-0"
              >
                {GENRES.map((cat) => (
                  <button
                    key={cat.id}
                ...
                {user && (
                  <>
                    <button
                      ...
                      className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs md:text-sm font-medium border flex items-center gap-1 ${
                        activeCategory === 'mylist'
                          ? 'bg-white text-black'
                          : 'bg-white/10 text-gray-300 border-transparent'
                      }`}
                    >
                      Listem
                    </button>
              ...
              <button
                ...
                className="absolute right-0 z-20 bg-gradient-to-l from-[#141414] to-transparent px-4 h-full hidden md:flex items-center opacity-0 group-hover:opacity-100 transition"
              >
              ...
              >
                ...
                {authMode === 'register' && (
                  <div className="relative">
                    <input
                      ...
                      onChange={handleInputChange}
                      className={`w-full bg-[#333] rounded px-4 py-3 text-white placeholder-gray-400 focus:bg-[#444] outline-none border transition text-sm ${
                ...
                {authMode === 'register' && (
                  ...
                  >
                    ...
                    >
                      {captchaVerified && (
                        <CheckSquare className="text-green-600 w-7 h-7" />
                      )}
                    </div>
                    <span className="text-[#222] text-sm font-medium">
                      Ben robot değilim
                    </span>
                ...
                <button
                  ...
                  className="w-full bg-red-600 hover:bg-red-700 disabled:bg-red-900 disabled:cursor-not-allowed text-white font-bold py-3 rounded mt-2 transition flex justify-center items-center gap-2"
                >
                  {authLoading && (
                ...
                <div className="relative my-4">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-black px-2 text-gray-500">veya</span>
                  </div>
                </div>
              </form>
        ...
        activeCategory !== 'history' && (
          <div className="relative h-[70vh] md:h-[85vh] w-full animate-in fade-in duration-700">
            <div className="absolute inset-0">
              <img
                src={getImage(heroMovie.backdrop_path, 'original')}
                alt="Hero"
            ...
            <div className="absolute bottom-[25%] left-4 md:left-10 max-w-3xl space-y-5 z-10">
              <h1 className="text-4xl md:text-7xl font-black drop-shadow-2xl leading-none tracking-tight">
                {heroMovie.title || heroMovie.name}
              </h1>
              <p className="text-sm md:text-xl text-gray-100 drop-shadow-lg line-clamp-3 font-medium">
                {heroMovie.overview}
              </p>
              <div className="flex items-center gap-3 pt-2">
                <button
                  onClick={() => handleMovieClick(heroMovie)}
                  className="bg-white text-black px-8 py-3 rounded-lg hover:bg-gray-200 transition flex items-center gap-2 font-bold text-lg shadow-lg"
                >
                  <Play fill="currentColor" size={24} /> Oynat
                </button>
                <button
                  onClick={(e) => toggleMyList(heroMovie, e)}
                  className="bg-gray-600/60 text-white px-6 py-3 rounded-lg hover:bg-gray-600/40 backdrop-blur-md transition flex items-center gap-2 font-bold text-lg border border-white/10"
                >
                  {isInList(heroMovie.id) ? (
                    <Check size={24} />
                  ) : (
                    <Plus size={24} />
                  )}{' '}
      ...
      {/* --- ANA İÇERİK --- */}
      <div
        className={`px-4 md:px-10 min-h-screen transition-all duration-500 ${
          !query &&
          heroMovie &&
          activeCategory !== 'mylist' &&
          activeCategory !== 'history'
      ...
      >
        ...
        {activeCategory === 'mylist' && (
          <div className="flex items-center gap-2 mb-6 text-gray-400">
            ...
            <span className="text-white font-bold text-2xl">Listem</span>
          </div>
        )}
        {activeCategory === 'history' && (
          <div className="flex items-center gap-2 mb-6 text-gray-400">
            ...
            {movies.length === 0 ? (
              <div className="text-center py-24 text-gray-500 flex flex-col items-center gap-4">
                ...
                <p className="text-2xl font-light">
                  Burada henüz bir içerik yok.
                </p>
                {activeCategory === 'mylist' && (
                  <p className="text-sm">
                    Filmleri beğenerek listenize ekleyebilirsiniz.
                  </p>
                )}
              </div>
            ) : (
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 backdrop-blur-[2px]">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="p-2 rounded-full border-2 border-gray-400 hover:border-white hover:bg-white/20 transition text-white"
                            title="Listeme Ekle"
                          >
                            {isInList(movie.id) ? (
                              <Check size={16} />
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-[#181818] w-full max-w-6xl h-[90vh] rounded-xl overflow-hidden shadow-2xl relative flex flex-col ring-1 ring-white/10">
            <button
              ...
              className="absolute top-4 right-4 z-50 bg-black/60 text-white p-2 rounded-full hover:bg-red-600 hover:text-white transition backdrop-blur-md group"
            >
              <X
                size={24}
            ...
            {isPlaying ? (
              <div className="w-full h-full bg-black">
                <iframe
                  src={getVideoSource(selectedMovie)}
              ...
              <div className="flex flex-col h-full overflow-y-auto custom-scrollbar">
                <div className="relative h-[55vh] shrink-0 w-full">
                  ...
                  <div className="absolute bottom-0 left-0 p-6 md:p-12 w-full z-10 flex flex-col gap-4">
                    <h2 className="text-4xl md:text-6xl font-black text-white drop-shadow-2xl leading-none tracking-tight">
                      {selectedMovie.title || selectedMovie.name}
                    </h2>
                    <div className="flex items-center gap-4 mt-2">
                      <button
                        onClick={startVideo}
                        className="bg-red-600 text-white px-8 py-3 rounded-lg font-bold hover:bg-red-700 hover:scale-105 transition transform flex items-center gap-3 shadow-xl shadow-red-900/40 text-lg"
                      >
                        <Play fill="currentColor" size={24} /> HEMEN İZLE
                      </button>
                      <button
                        onClick={(e) => toggleMyList(selectedMovie, e)}
                        className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-4 py-3 rounded-lg font-bold hover:bg-white/20 transition flex items-center gap-2"
                      >
                        {isInList(selectedMovie.id) ? (
                          <Check size={24} />
                        ) : (
                          <Plus size={24} />
                        )}
                ...
                <div className="p-6 md:p-12 grid md:grid-cols-[2fr_1fr] gap-10 bg-[#181818] flex-grow">
                  <div>
                    <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2">
                      <Info size={20} className="text-red-500" /> Özet
                    </h3>
                    <p className="text-gray-300 leading-relaxed text-lg font-light">
                      {selectedMovie.overview || 'Özet bulunamadı.'}
                    </p>
                  </div>
                  <div className="space-y-4 bg-[#202020] p-6 rounded-xl h-fit border border-white/5">
                    ...
                    <div className="flex justify-between items-center border-b border-gray-700 pb-3">
                      ...
                      <span className="uppercase text-white font-medium bg-white/10 px-2 py-0.5 rounded text-xs">
      ...
      <footer className="py-8 text-center text-gray-500 text-xs mt-12 border-t border-gray-800 bg-[#141414]">
        <p>
          &copy; 2025 <span className="text-red-600 font-bold">CineAlp</span>.
          Tüm hakları saklıdır.
        </p>
        <p className="mt-2 flex items-center justify-center gap-2">
          Geliştirici:{' '}
          <span className="text-white font-medium border border-gray-700 px-2 py-0.5 rounded bg-gray-800">
            Alperen Ceylan
          </span>
        </p>
      </footer>
